...
public class ModelAssimp : IDisposable
{
    private readonly Assimp        _assimp;
    private          List<Texture> _texturesLoaded = new();

    public ModelAssimp(string path, bool gamma = false)
    {
    ...
    private unsafe void ProcessNode(Node* node, Scene* scene)
    {
        ...
        for (var i = 0; i < node->MNumChildren; i++) ProcessNode(node->MChildren[i], scene);
    }
    ...
    private unsafe List<Texture> LoadMaterialTextures(Material* mat, TextureType type, string typeName)
    {
        ...
        for (uint i = 0; i < textureCount; i++)
        {
            ...
            if (!skip)
            {
                var texture = new Texture(Path.Combine(Path.GetDirectoryName(Directory) ?? string.Empty, path), type);
                texture.Path = path;
    ...
    private uint[] BuildIndices(List<uint> indexCollection)
    {
